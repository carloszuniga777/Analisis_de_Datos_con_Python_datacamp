...
# Si das una única estimación de un estadístico, seguro que te equivocas en alguna medida. 
# Por ejemplo, la proporción hipotética de envíos retrasados era del 6 %. 
# 
# Aunque las pruebas sugieran la hipótesis nula de que la proporción de envíos retrasados es igual a esto, 
# para cualquier nueva muestra de envíos es probable que la proporción sea diferente, 
# debido a la variabilidad del muestreo. Por ello, es buena idea definir un intervalo de confianza. 
# Es decir, indicas lo siguiente: "tenemos un 95 % de 'confianza' en que la proporción de envíos retrasados 
# está entre A y B" (para algún valor de A y B).
#
# El muestreo en Python ha demostrado dos métodos para calcular intervalos de confianza. 
# Aquí utilizarás los cuantiles de la distribución bootstrap para calcular el intervalo de confianza.
...
#-------------------------------------------
# Parte 4: Calculo de Intervalo de confianza
#-------------------------------------------
# Calcula un intervalo de confianza del 95 % a partir de late_shipments_boot_distn con el método cuantil, 
# etiquetando los intervalos inferior y superior lower y upper.
...
print([x.item() for x in [lower, upper]])  # [0.052, 0.098]

# Interpretación en tu ejercicio
#
# En el ejemplo, el resultado fue:
#
#                   IC 95 % = (0.052, 0.098)
#
# Esto se lee así:
# 
# “Con un 95 % de confianza estadística, la proporción real de envíos retrasados está entre 5,3 % y 9,8 %.”
#
# Como el valor 0.06 (6 %) —la hipótesis inicial (H0)— está dentro de ese intervalo:
#   - No hay evidencia suficiente para rechazar la hipótesis de que el retraso sea del 6 %.
#   - Esto es consistente con un valor p alto en la prueba de hipótesis.

# 📌 Resumen rápido
# Cuantiles → marcan cortes en tu distribución simulada.
# Para qué → armar un intervalo de confianza con bootstrap.
# Cómo interpretar → el rango muestra los valores plausibles del parámetro; si tu hipótesis cae dentro, es compatible con tus datos.